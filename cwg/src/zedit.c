...
void zedit_disp_menu(struct descriptor_data *d)
{
  ...
  while (MYCMD.command != 'S') {
    ...
    switch (MYCMD.command) {
    case 'M':
      write_to_output(d, "%sLoad %s [%s%d%s], Max : %d, MaxR %d, Chance %d",
              MYCMD.if_flag ? " then " : "",
              mob_proto[MYCMD.arg1].player.short_descr, cyn,
              mob_index[MYCMD.arg1].vnum, yel, MYCMD.arg2, MYCMD.arg4, MYCMD.arg5
              );
      break;
    case 'G':
...
/*-------------------------------------------------------------------*/
  case ZEDIT_ARG5:
    ...
    default:
       ...
       */
      cleanup_olc(d, CLEANUP_ALL);
      mudlog(BRF, LVL_BUILDER, TRUE, "SYSERR: OLC: zedit_parse(): case ARG5: Ack!");
      write_to_output(d, "Oops...\r\n");
      break;
    }
    break;
...
/*-------------------------------------------------------------------*/
  case ZEDIT_MIN_LEVEL:
    ...
    pos = atoi(arg);
    if (!isdigit(*arg) || pos < 0 || pos > LVL_IMPL)
      write_to_output(d, "Try again (0 - LVL_IMPL) : \r\n");
    else {
