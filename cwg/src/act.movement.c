...
int do_simple_move(struct char_data *ch, int dir, int need_specials_check)
{
  ...
  if ((GET_LEVEL(ch) < LVL_IMMORT) && (GET_LEVEL(ch) > ZONE_MAXLVL(rm->zone)) &&
    (ZONE_MAXLVL(rm->zone) > 0)) {
    send_to_char(ch, "Sorry, you are too high a level to enter this zone.\r\n");
    return(0);
  }

  if ((GET_LEVEL(ch) < LVL_IMMORT) && ZONE_FLAGGED(rm->zone, ZONE_CLOSED)){
  ...
  if ((GET_REAL_LEVEL(ch) >= LVL_IMMORT && GET_REAL_LEVEL(ch) < LVL_GOD) &&
...
ACMD(do_pose)
{
    send_to_char(ch, "You strike a pose! Well, you would if this command worked.\r\n");
}
