...
ACMD(do_say)
{
  ...
  if(!*argument) {
    ...
    if (!IS_NPC(ch) && PRF_FLAGGED(ch, PRF_NOREPEAT)) {
      ...
      send_to_char(ch, "@WYou @G%s, @W'@C%s@W'@n\r\n", verb, argument);
    }
  ...
  if(!*argument) {
    ...
    for (tch = world[IN_ROOM(ch)].people; tch; tch = tch->next_in_room) {
      ...
      if (tch != ch && tch->desc) {
        ...
        send_to_char(tch, "@W%s @G%ss%s out of character, @W'@C%s@W'@n\r\n", CAN_SEE(tch, ch) ? GET_NAME(ch) : "Someone", verb, GET_SKILL(tch, SPEAKING(ch)) ? "" : ", in an unfamiliar tongue,", obuf);
        } else {
          send_to_char(tch, "@W%s @G%ss out of character, @W'@C%s@W'@n\r\n", CAN_SEE(tch, ch) ? GET_NAME(ch) : "Someone", verb, argument);
        }
      }
    }
    if (!IS_NPC(ch) && PRF_FLAGGED(ch, PRF_NOREPEAT)) {
      ...
      delete_doubledollar(argument);
      send_to_char(ch, "@WYou @G%s out of character, @W'@C%s@W'@n\r\n", verb, argument);
    }
    ...
    for (f = k->followers; f; f = f->next)
      if (AFF_FLAGGED(f->follower, AFF_GROUP) && (f->follower != ch) && AWAKE(f->follower)) {
        if (!IS_NPC(ch) && !IS_NPC(f->follower) && CONFIG_ENABLE_LANGUAGES) {
          ...
          garble_text(buf, 100, MIN_LANGUAGES);
        }
        if (CONFIG_ENABLE_LANGUAGES) {
...
void perform_tell(struct char_data *ch, struct char_data *vict, char *arg)
{
  ...
  if (CONFIG_ENABLE_LANGUAGES) {
    send_to_char(vict, "@R%s @rtells @Wyou%s '@C%s@W'@n%s\r\n", CAN_SEE(vict, ch) ? GET_NAME(ch) : "Someone", GET_SKILL(vict, SPEAKING(ch)) ? "," : ", in an unfamiliar tongue,", buf, CCNRM(vict, C_NRM));
  } else {
    send_to_char(vict, "@R%s @rtells @Wyou '@C%s@W'@n%s\r\n", CAN_SEE(vict, ch) ? GET_NAME(ch) : "Someone", buf, CCNRM(vict, C_NRM));
  }

  if (!IS_NPC(ch) && PRF_FLAGGED(ch, PRF_NOREPEAT))
    send_to_char(ch, "%s", CONFIG_OK);
  else {
    send_to_char(ch, "@WYou @Rtell @Y%s@W, '@C%s@W'@n%s\r\n", GET_NAME(vict), arg, CCNRM(ch, C_CMP));
  }
  ...
  for (i = descriptor_list; i; i = i->next) {
	...
	continue;
      ...
      if (CONFIG_ENABLE_LANGUAGES) {
        ...
        snprintf(buf1, sizeof(buf1), "$n %ss%s '%s'", com_msgs[subcmd][1], GET_SKILL(i->character, SPEAKING(ch)) ? "," : ", in an unfamiliar tongue,", argument);
      } else {
